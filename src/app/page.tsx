  ...
  return (
    <>
      ...
      <main className="overflow-hidden">
        <section id="inicio" className="mx-auto max-w-7xl px-4 pb-18 pt-8 sm:px-6 lg:px-8 lg:pb-24 lg:pt-10">
          <div className="grid gap-10 lg:grid-cols-[0.84fr_1.16fr] lg:items-center lg:gap-14">
            <div className="relative z-10 max-w-[35rem] lg:py-10">
              <p className="text-[0.72rem] font-semibold uppercase tracking-[0.32em] text-[#8A5A3C]">
                {site.locationLabel}
              </p>
              <h1 className="mt-6 max-w-xl font-editorial text-[3.25rem] leading-[0.98] tracking-[-0.03em] text-[#1F1A17] sm:text-[3.8rem] lg:text-[4.3rem]">
                {site.heroTitle}
              </h1>
              <p className="mt-7 max-w-lg text-base leading-8 text-[#6D655E] sm:text-lg">
                {site.heroSubtitle}
              </p>
          ...
          <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
            ...
            <div className="mt-10 grid gap-4 xl:grid-cols-4">
              {processSteps.map((step, index) => (
                <article
                  key={step.number}
                  className="relative overflow-hidden rounded-[1.9rem] border border-[#E8DDD2] bg-[#FFFDF8] px-5 py-6"
                >
                  {index < processSteps.length - 1 ? (
                    <span className="absolute right-0 top-8 hidden h-px w-10 translate-x-1/2 bg-[#D8C1A8] xl:block" />
                  ) : null}
                  <span className="text-[0.72rem] font-semibold tracking-[0.28em] text-[#8A5A3C]">
                    {step.number}
                  </span>
                  <h3 className="mt-5 font-editorial text-[1.7rem] leading-none text-[#1F1A17]">
                    {step.title}
                  </h3>
